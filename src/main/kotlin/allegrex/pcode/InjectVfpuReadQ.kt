...
class InjectVfpuReadQ(
  ...
  override fun getPcode(program: Program, con: InjectContext): Array<PcodeOp> {
    ...
    pCode.emitPieceRegisters(output, VfpuPcode.regIdToName(baseRegId + stride * 3), VfpuPcode.regIdToName(baseRegId + stride * 2))
    pCode.emitPieceVarnodeRegister(output, output, VfpuPcode.regIdToName(baseRegId + stride))
